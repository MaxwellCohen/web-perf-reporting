...
vi.mock("@/components/ui/select", () => ({
  Select: ({
    ...
    <div
      ...
      onChange={(event) => onValueChange((event.target as unknown as HTMLSelectElement).value)}
    >
      {children}
    </div>
  ),
...
describe("ChartSelector", () => {
  ...
  it("normalizes string options and uses the first option as the placeholder", () => {
    ...
    const { container } = render(
      <ChartSelector id="chart-selector" options={["LCP", "CLS"]} onValueChange={onValueChange} />,
    );
  ...
  it("supports labeled option objects", () => {
    ...
    expect(container.querySelector('[data-testid="mock-trigger"]')).toHaveTextContent(
      "Largest Contentful Paint",
    );
    expect(container.querySelector('option[value="CLS"]')).toHaveTextContent(
