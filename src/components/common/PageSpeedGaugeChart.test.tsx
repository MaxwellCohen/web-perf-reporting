...
vi.mock('recharts', () => ({
  ...
  Label: ({
    ...
    content: (props: { viewBox?: { cx: number; cy: number; innerRadius?: number; outerRadius?: number } }) => React.ReactNode;
  }) => {
...
describe('GaugeChart', () => {
  ...
  it('renders chart without metric label when metric is empty', () => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
    const { container } = render(
      <GaugeChart metric="" data={validMetricData as any} />
    );
    expect(container.textContent).toContain('1,200');
...
describe('HorizontalGaugeChart', () => {
  ...
  it('renders without metric when metric is undefined', () => {
    const { container } = render(
      <HorizontalGaugeChart data={validMetricData as any} />
    );
    expect(container.querySelector('svg')).toBeTruthy();
  });
...
describe('LineChart', () => {
  ...
  it('renders SVG with segments', () => {
    const { container } = render(
      <LineChart chartData={baseChartData as any} value={150} />
    );
    expect(container.querySelector('svg')).toBeTruthy();
  ...
  it('renders when value equals maxValue', () => {
    const { container } = render(
      <LineChart chartData={baseChartData as any} value={600} />
    );
    expect(container.querySelector('svg')).toBeTruthy();
  });
  ...
  it('renders when value is 0', () => {
    const { container } = render(
      <LineChart chartData={baseChartData as any} value={0} />
    );
    expect(container.querySelector('svg')).toBeTruthy();
  });
  ...
  it('formats small values correctly when value <= 1', () => {
    ...
    const { container } = render(
      <LineChart chartData={smallData as any} value={0.5} />
    );
    expect(container.querySelector('svg')).toBeTruthy();
  ...
  it('formats values with toFixed when value > 1', () => {
    const { container } = render(
      <LineChart chartData={baseChartData as any} value={200} />
    );
  ...
  it('renders indicator with width 4 when value equals maxValue', () => {
    const { container } = render(
      <LineChart chartData={baseChartData as any} value={300} />
    );
    const rects = container.querySelectorAll('rect');
    const indicatorRect = Array.from(rects).find(
      (r) => r.getAttribute('class')?.includes('rounded-l-full')
    );
...
describe('HorizontalScoreChart', () => {
  ...
  it('renders with className prop', () => {
    const { container } = render(
      <HorizontalScoreChart score={0.5} className="custom-class" />
    );
    expect(container.querySelector('svg')).toBeTruthy();
  });
