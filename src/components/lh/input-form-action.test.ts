...
describe('fetchJsonFromUrl', () => {
  ...
  it('throws when response is not ok', async () => {
    ...
    await expect(
      fetchJsonFromUrl('https://example.com/data.json', mockFetch),
    ).rejects.toThrow('Failed to fetch JSON from URL: Not Found');
  });
  ...
  it('throws when response is not valid JSON', async () => {
    ...
    await expect(
      fetchJsonFromUrl('https://example.com/data.json', mockFetch),
    ).rejects.toThrow('URL did not return valid JSON');
  });
});
