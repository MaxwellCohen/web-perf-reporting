...
} from '@/components/page-speed/shared/dataSortingHelpers';

describe('sortByMaxValue', () => {
  it('returns data unchanged when numReports <= 1', () => {
    const data = [{ id: 'a', val: 10 }, { id: 'b', val: 20 }];
    const result = sortByMaxValue(data, (d) => d.id, (d) => d.val, 1);
  ...
  it('sorts by max value per key across reports', () => {
    ...
    const result = sortByMaxValue(data, (d) => d.id, (d) => d.val, 2);
