...
import { JSONTextArea } from '@/components/page-speed/viewer/TextImport';
...
describe('JSONTextArea', () => {
  it('renders summary and textarea', () => {
    const { container } = render(
      <JSONTextArea data={[]} onUpdate={vi.fn()} />
    );
  ...
  it('shows initial data when provided', () => {
    const { container } = render(
      <JSONTextArea data={[mockData as any]} onUpdate={vi.fn()} />
    );
  ...
  it('calls onUpdate with parsed data when Show Report clicked', () => {
    ...
    const { container } = render(
      <JSONTextArea data={[mockData as any]} onUpdate={onUpdate} />
    );
