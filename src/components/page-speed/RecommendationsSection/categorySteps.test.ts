...
} from '@/components/page-speed/RecommendationsSection/categorySteps';

describe('categorySteps', () => {
  it('maps audit ids to the expected categories', () => {
    expect(getCategoryFromAuditId('render-blocking-resources')).toBe(
      'Render Blocking',
    );
    ...
    expect(getCategoryFromAuditId('javascript-execution-time')).toBe(
      'JavaScript',
    );
    expect(getCategoryFromAuditId('server-response-time')).toBe('Server');
    expect(getCategoryFromAuditId('preconnect-to-required-origins')).toBe(
      'Resource Hints',
    );
  ...
  it('returns specific generic steps for known audit categories', () => {
    expect(
      getGenericStepsForCategory('render-blocking-resources', []).map(
        ({ step }) => step,
      ),
    ).toContain('Inline critical CSS');

    expect(
      getGenericStepsForCategory('javascript-execution-time', []).map(
        ({ step }) => step,
      ),
    ).toContain('Remove unused dependencies');

    expect(
      getGenericStepsForCategory('redirects', []).map(({ step }) => step),
    ).toEqual([
  ...
  it('returns default steps and recommendation review guidance for unknown audits', () => {
    expect(
      getGenericStepsForCategory('custom-audit', []).map(({ step }) => step),
    ).toEqual([
    ...
    expect(
      getGenericStepsForCategory('custom-audit', [
        { step: 'Use a CDN', reports: [] },
      ]).map(({ step }) => step),
    ).toEqual([
      'Test changes in a staging environment',
      'Monitor performance metrics after implementation',
  ...
  it('returns both category and steps together', () => {
    expect(
      getCategoryAndGenericSteps('unused-javascript', []).category,
    ).toBe('JavaScript');
    expect(
      getCategoryAndGenericSteps('unused-javascript', []).genericSteps.length,
    ).toBeGreaterThan(0);
  });
});
