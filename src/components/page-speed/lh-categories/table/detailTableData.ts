...
export function hasDetailSubitems(rows: DetailTableItem[]): boolean {
  return rows.some((row) =>
    (row.auditResult?.details?.items || []).some(
      (item) => (item.subItems?.items || []).length > 0,
    ),
  );
}

export function flattenDetailItems(
  rows: DetailTableItem[],
): Array<TableItem | OpportunityItem> {
...
export function groupRowsByReportLabel(
  rows: DetailTableItem[],
): Map<string, DetailTableItem[]> {
