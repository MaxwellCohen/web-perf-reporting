...
} from '@/components/page-speed/lh-categories/table/utils';
...
describe('utils', () => {
  ...
  describe('getDerivedSubItemsHeading', () => {
    it('returns null when no subItemsHeading', () => {
      expect(
        getDerivedSubItemsHeading({ key: 'x', valueType: 'text', label: 'X' }),
      ).toBeNull();
    });
  ...
  describe('mergedTable', () => {
    ...
    it('returns mobile items and device when only mobile provided', () => {
      ...
      const [, items, device] = mergedTable(
        undefined,
        mobileItems,
        mobileHeadings,
        undefined,
    ...
    it('merges when hasNode and both desktop and mobile headings', () => {
      ...
      const headings: TableColumnHeading[] = [
        { key: 'id', valueType: 'node', label: 'Node' },
      ];
  ...
  describe('mergeHeadings', () => {
    it('interleaves mobile and desktop headings with device suffix when summable', () => {
      const mobile = [
        { key: 'm1', valueType: 'bytes', label: 'M1' },
      ] as TableColumnHeading[];
      const desktop = [
        { key: 'd1', valueType: 'bytes', label: 'D1' },
      ] as TableColumnHeading[];
