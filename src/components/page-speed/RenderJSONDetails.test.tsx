...
describe('RenderJSONDetails', () => {
  ...
  it('renders details with custom title', () => {
    const { container } = render(
      <RenderJSONDetails data={{}} title="Custom Title" />,
    );
    expect(container.firstChild).toMatchSnapshot();
  });
  ...
  it('renders data2 when provided', () => {
    ...
    const data2 = { b: 2 };
    const { container } = render(
      <RenderJSONDetails data={data} data2={data2} />,
    );
    expect(container.firstChild).toMatchSnapshot();
  });
  ...
  it('can be expanded to show content', () => {
    const { container } = render(
      <RenderJSONDetails data={{ nested: { value: 1 } }} />,
    );
  ...
  it('passes through additional props to details element', () => {
    const { container } = render(
      <RenderJSONDetails data={{}} data-testid="json-details" open />,
    );
    expect(container.firstChild).toMatchSnapshot();
  });
});
