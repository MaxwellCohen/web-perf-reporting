...
describe('useFetchPageSpeedData', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it('returns defaultData and isLoading false when hasDefaultData', () => {
    ...
    const { result } = renderHook(
      () => useFetchPageSpeedData('https://example.com', defaultData),
      {
  ...
  it('returns defaultData when defaultData has items (filtered)', () => {
    ...
    const { result } = renderHook(
      () => useFetchPageSpeedData('', defaultData),
      {
        wrapper: createWrapper(),
      },
    );
  ...
  it('does not use defaultData when it is empty', async () => {
    ...
    const { result } = renderHook(
      () => useFetchPageSpeedData('https://example.com', []),
      {
        wrapper: createWrapper(),
      },
    );
