...
} from '@/components/page-speed/JSUsage/jsUsageTableFilters';
...
vi.mock('@/components/page-speed/lh-categories/table/RenderTableValue', () => ({
  ...
  RenderTableValue: ({
    ...
    value: number;
    heading: string | null;
  }) => <span data-testid="table-value">{heading ?? ''}: {value}</span>,
}));
...
describe('jsUsageTableFilters', () => {
  describe('formatFilterValue', () => {
    ...
    it('renders ms for time column', () => {
      const { container } = render(
        <div>{formatFilterValue(500, 'scripting', 'Scripting')}</div>,
      );
    ...
    it('renders number for other columns', () => {
      const { container } = render(
        <div>{formatFilterValue(1234, 'count', 'Count')}</div>,
      );
  ...
  describe('RangeFilter', () => {
    ...
    function RangeFilterWrapper() {
      const data = [{ id: 1, value: 50 }, { id: 2, value: 100 }];
      const cols = [
