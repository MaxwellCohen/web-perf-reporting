...
describe("recommendationHelpers", () => {
  describe("getRecommendationAuditId", () => {
    it("strips last segment after final hyphen", () => {
      expect(getRecommendationAuditId("unused-css-rules-metric")).toBe("unused-css-rules");
    });
  ...
  describe("isNetworkDependencyTreeRecommendation", () => {
    it("returns true for id starting with network-dependency-tree-insight", () => {
      expect(isNetworkDependencyTreeRecommendation("network-dependency-tree-insight")).toBe(true);
      expect(
        isNetworkDependencyTreeRecommendation("network-dependency-tree-insight-something"),
      ).toBe(true);
    });

    it("returns false for other ids", () => {
      expect(isNetworkDependencyTreeRecommendation("unused-css-rules")).toBe(false);
      expect(isNetworkDependencyTreeRecommendation("other-network-dependency")).toBe(false);
    });
  });
});
