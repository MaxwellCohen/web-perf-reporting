...
describe('RenderTableValue', () => {
  ...
  it('returns null for null', () => {
    const { container } = render(<RenderTableValue value={null as unknown as ItemValue} device="" />);
    expect(container.firstChild).toBeNull();
  });

  it('renders bytes with heading valueType', () => {
    const { container } = render(
      <RenderTableValue value={2048} heading={{ key: 'size', label: 'Size', valueType: 'bytes' }} device="" />
    );
    expect(container.textContent).toContain('2.00 KB');
  });
  ...
  it('renders text with heading valueType', () => {
    const { container } = render(
      <RenderTableValue value="hello" heading={{ key: 'text', label: 'Text', valueType: 'text' }} device="" />
    );
  ...
  it('renders code with heading valueType', () => {
    const { container } = render(
      <RenderTableValue value="const x = 1" heading={{ key: 'code', label: 'Code', valueType: 'code' }} device="" />
    );
