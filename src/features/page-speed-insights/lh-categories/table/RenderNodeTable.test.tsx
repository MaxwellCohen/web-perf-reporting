...
import {
  RenderNodeTable,
  NodeSummary,
} from "@/features/page-speed-insights/lh-categories/table/RenderNodeTable";
...
describe("RenderNodeTable", () => {
  it("returns null when no node header", () => {
    ...
    const { container } = render(
      <RenderNodeTable headings={headings as any} items={[]} device="Desktop" title="Test" />,
    );
...
describe("NodeSummary", () => {
  it("returns null for null item", () => {
    const { container } = render(<NodeSummary item={null as any} device="Desktop" />);
    expect(container.firstChild).toBeNull();
  });

  it("renders text type with value", () => {
    const item = { type: "text", value: "Heading Text" };
    const { container } = render(<NodeSummary item={item as any} device="Mobile" />);
  ...
  it("renders nodeLabel and snippet", () => {
    ...
    const { container } = render(<NodeSummary item={item as any} device="Desktop" />);
