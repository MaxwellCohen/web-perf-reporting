...
vi.mock("@/features/page-speed-insights/lh-categories/RenderChecklist", () => ({
  RenderChecklist: () => <div data-testid="checklist" />,
}));

vi.mock("@/features/page-speed-insights/lh-categories/table/RenderTable", () => ({
  DetailTable: () => <div data-testid="detail-table" />,
}));

vi.mock("@/features/page-speed-insights/lh-categories/RenderDebugdata", () => ({
  RenderDebugData: () => <div data-testid="debug-data" />,
}));

vi.mock("@/features/page-speed-insights/lh-categories/table/RenderCriticalChain", () => ({
...
vi.mock("@/features/page-speed-insights/lh-categories/table/RenderNetworkDependencyTree", () => ({
  RenderNetworkDependencyTree: () => <div data-testid="network-tree" />,
}));

function makeItem(detailsType: string, details: unknown, userLabel = "Mobile"): TableDataItem {
  return {
...
describe("RenderDetails", () => {
  it("returns null when items have mixed detail types", () => {
    const items = [makeItem("table", {}), makeItem("list", {})];
  ...
  it("returns null when items have no detail type", () => {
    const items = [
      {
        ...makeItem("table", {}),
        auditResult: { ...makeItem("table", {}).auditResult, details: {} },
      } as any,
    ];
    const { container } = render(<RenderDetails items={items} />);
    expect(container.firstChild).toBeNull();
  ...
  it("renders DetailTable for table type", () => {
    const items = [makeItem("table", {}), makeItem("table", {})];
    const { container } = render(<RenderDetails items={items} />);
    expect(container.querySelector('[data-testid="detail-table"]')).toBeTruthy();
  });

  it("returns null for screenshot type", () => {
    const items = [makeItem("screenshot", {}), makeItem("screenshot", {})];
    const { container } = render(<RenderDetails items={items} />);
    expect(container.firstChild).toBeNull();
  });

  it("renders RenderDebugData for debugdata type", () => {
    const items = [makeItem("debugdata", {}), makeItem("debugdata", {})];
    const { container } = render(<RenderDetails items={items} />);
    expect(container.querySelector('[data-testid="debug-data"]')).toBeTruthy();
  });

  it("returns null for unknown detail type", () => {
    const items = [makeItem("unknown" as any, {}), makeItem("unknown" as any, {})];
    const { container } = render(<RenderDetails items={items} />);
    expect(container.firstChild).toBeNull();
  });
});
