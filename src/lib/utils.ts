import { CruxDate, CruxHistoryItem, CruxHistoryReport, CruxReport } from "@/lib/schema"
...
export function formatCruxReport(item: CruxReport): CruxHistoryItem[] | null {
  ...
  return metrics.map((metric) => {
    ...
    const d: CruxHistoryItem = {
      url: item.urlNormalizationDetails?.normalizedUrl ?? item.record.key.origin ?? '',
...
export function formatCruxHistoryReport(item: CruxHistoryReport): CruxHistoryItem[] | null {
  const url = item?.urlNormalizationDetails?.normalizedUrl ?? item?.record?.key?.origin;
  if (!url) return null;
  return metrics.map((metric) => {
    const data = item?.record?.metrics[metric];
  ...
  }).flatMap(i => i).filter((item): item is CruxHistoryItem => item !== null);
}