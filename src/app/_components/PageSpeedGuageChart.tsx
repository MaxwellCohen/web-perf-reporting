...
export function GaugeChart({ metric, data }: { metric: string, data?: UserPageLoadMetricV5 }) {
    ...
    const chartData = [
        { name: chartConfig['poor'].label, value: Math.max(distributions[2].min * 1.10 || 0, value), fill: "var(--color-poor)" },
        { name: chartConfig['ni'].label, value: distributions[1].max, fill: "var(--color-ni)" },
        { name: chartConfig['good'].label, value: distributions[0].max, fill: "var(--color-good)" },
    ];
