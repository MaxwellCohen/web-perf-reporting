...
vi.mock("@/features/page-speed-insights/lh-categories/table/RenderTableCell", () => ({
  RenderTableCell: ({ value, heading }: { value: unknown; heading: { key?: string } }) => (
...
vi.mock("@/features/page-speed-insights/lh-categories/table/RenderTableValue", () => ({
  RenderTableValue: ({ value }: { value: unknown }) => <span>{String(value ?? "")}</span>,
}));

vi.mock("@/features/page-speed-insights/lh-categories/table/RenderTableRowContainer", () => ({
  RenderTableRowContainer: ({ children }: { children: React.ReactNode }) => (
...
vi.mock("@/features/page-speed-insights/lh-categories/table/itemDevice", () => ({
  getItemDevice: (item: { _device?: string }, device: string) => item._device ?? device,
}));

vi.mock("@/components/ui/accordion", () => ({
  Details: ({ children, className }: { children: React.ReactNode; className?: string }) => (
...
describe("NestedTableNoCollapse", () => {
  it("renders main row and subitems when present", () => {
    ...
    const { container } = render(
      <NestedTableNoCollapse item={item as any} headings={subHeadings as any} device="Desktop" />,
    );
...
describe("NestedTable", () => {
  it("renders item in details/summary", () => {
    ...
    const { container } = render(
      <NestedTable item={item as any} headings={headings as any} device="Desktop" />,
    );
...
describe("RenderSubItemsHeader", () => {
  it("renders subheadings when subItemsHeading present", () => {
    ...
    const { container } = render(<RenderSubItemsHeader headings={subHeadings as any} />);
