...
describe("networkRequestsTable", () => {
  ...
  it("sorts headings by preferred key order and keeps extras afterward", () => {
    ...
    expect(
      sortHeadingsByKeyOrder(headings, NETWORK_REQUESTS_COLUMN_ORDER).map((heading) => heading.key),
  ...
  it("returns null when the timing range is invalid or incomplete", () => {
    expect(
      getNetworkRequestsTimeRange([{ networkRequestTime: 100, networkEndTime: 100 }]),
    ).toBeNull();
    ...
    expect(
      getNetworkRequestsTimeRange([{ networkRequestTime: undefined, networkEndTime: undefined }]),
    ).toBeNull();
  });
