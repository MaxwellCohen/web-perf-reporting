...
describe("extractSpecificRecommendations", () => {
  ...
  it("adds explanation-based steps for third-party", () => {
    const result = extractSpecificRecommendations("audit", [], {
      explanation: "This page includes third-party scripts that block rendering.",
    });
    expect(
      result.some((r) => r.step.includes("lazy loading") || r.step.includes("third-party")),
    ).toBe(true);
  });
  ...
  it("adds explanation-based steps for render-blocking", () => {
    ...
    expect(result.some((r) => r.step.includes("Defer") || r.step.includes("render-blocking"))).toBe(
      true,
    );
  });
